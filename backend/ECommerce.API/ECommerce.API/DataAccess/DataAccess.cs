    ...
    public class DataAccess : IDataAccess
    {
        ...
        /*  public bool RemoveCartItem(int userId, int productId)
          {
              using (SqlConnection connection = new(dbconnection))
              {
                  ...
                  if (count == 0)
                  {
                      query = "DELETE FROM Carts (UserId, Ordered, OrderedOn) VALUES (" + userId + ", 'false', '');";
                      command.CommandText = query;
                      command.ExecuteNonQuery();
                  }
                  ...
                  query = "DELETE FROM CartItems (CartId, ProductId) VALUES (" + cartId + ", " + productId + ");";
                  command.CommandText = query;
                  command.ExecuteNonQuery();
                  return true;
              }
        ...
        public bool RemoveCartItem(int userId, int productId)
        {
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                // Step 1: Find the cart for the user with 'Ordered' as 'false'
                string query = "SELECT TOP 1 CartId FROM Carts WHERE UserId = @userId AND Ordered = 'false';";
                using (SqlCommand command = new(query, connection))
                {
                    ...
                    if (cartIdObj != null)
                    {
                        ...
                        // Step 2: Remove the item from CartItems
                        query = "DELETE FROM CartItems WHERE CartId = @cartId AND ProductId = @productId;";
                        using (SqlCommand removeItemCommand = new(query, connection))
                        {
                            removeItemCommand.Parameters.AddWithValue("@cartId", cartId);
                            removeItemCommand.Parameters.AddWithValue("@productId", productId);
                            int rowsAffected = removeItemCommand.ExecuteNonQuery();

                            if (rowsAffected > 0)
                            {
                                // Successfully removed the item from CartItems
                                return true;
                            }
                        }
                    }
